-- Phase 1: Critical Database Integrity Fixes

-- 1. Add 'publishing' status to listings enum for intermediate state
ALTER TYPE publish_status_enum ADD VALUE 'publishing';

-- 2. Add unique constraint on rooms slug to prevent collisions
CREATE UNIQUE INDEX IF NOT EXISTS idx_rooms_slug_unique ON public.rooms (slug);

-- 3. Create function to generate Greek-safe slugs with transliteration
CREATE OR REPLACE FUNCTION public.generate_greek_safe_slug(input_text text, listing_id uuid DEFAULT NULL)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF length(base_slug) < 3 THEN
    base_slug := 'listing-' || substring(listing_id::text from 1 for 8);
  END IF;
  
  -- Check for uniqueness and add counter if needed
  final_slug := base_slug;
  WHILE EXISTS (SELECT 1 FROM public.rooms WHERE slug = final_slug AND (listing_id IS NULL OR listing_id != listing_id)) LOOP
    counter := counter + 1;
...
-- 4. Create trigger to prevent publishing without room and ensure slug safety
...
-- 5. Create function for atomic publishing with transaction safety
