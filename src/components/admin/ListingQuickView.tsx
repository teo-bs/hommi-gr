...
export function ListingQuickView({ listing, open, onClose, onApprove, onReject, editMode, onSaveEdit }: ListingQuickViewProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{editMode ? 'Επεξεργασία Αγγελίας' : listing.title}</DialogTitle>
          <DialogDescription>
            {!editMode && `Αγγελία από ${listing.owner?.display_name || 'Unknown'}`}
          </DialogDescription>
        </DialogHeader>
